...
-- ============================================================
-- ДОБАВЛЕНО ПОСЛЕ v2 (фото водителя, чат по заказу, история,
-- время действий по остаткам)
-- ============================================================

alter table app_users add column if not exists photo_url text;
alter table orders add column if not exists hidden_from_history boolean not null default false;
alter table leftovers add column if not exists intercepted_at timestamptz;
alter table leftovers add column if not exists completed_at timestamptz;

-- Бакет для фото водителей (публичный, чтобы фото открывались всем).
...
-- orders: + hidden_from_history
-- leftovers: + intercepted_at, completed_at
-- app_users: + photo_url
-- Storage: бакет "avatars" (Storage -> Buckets)
-- Новая таблица: order_messages
